       ...
       if FORCE_SUB_CHANNEL1:
        ...
        if FORCE_SUB_GROUP1:
            try:
                ...
                self.LOGGER(__name__).warning(
                    f"Silakan periksa kembali var FORCE_SUB_GROUP dan Pastikan Bot anda Admin di Channel dengan izin link invite Pengguna melalui link undangan, Subs Group Saat Ini: {FORCE_SUB_GROUP1}"
                )
                self.LOGGER(__name__).info(
                    "\nBot Berhenti. Gabung Group https://t.me/jasadeploy untuk Bantuan"
                )
                sys.exit()
        try:
